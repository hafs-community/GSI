subroutine read_tms(mype,val_tovs,ithin,isfcalc,&
     ...
     dval_use)
! subprogram:    read_tms                 read bufr format tropics data
! prgmmr :   mjkim         org: nesdis               date: 2022-05-01
! code copied from read_saphir.f90
!
! abstract:  This routine reads BUFR format TROPICS radiance (brightness
!            temperature) data. 
!
...
!
! program history log:
!  2022-05-01  mjkim  - adapted from read_saphir.f90
!
!   input argument list:
!     mype     - mpi task id
...
! Declare local parameters
  ...
  integer(i_kind),parameter:: n2bhdr=6
...
! Declare local variables
  logical               :: outside,iuse,assim,valid, tmstab
  character(8)          :: subset
  ...
  integer(i_kind)       :: iskip, nbad_lza 
  integer(i_kind)       :: lnbufr, lnbufrtab, ksatid,isflg  
  integer(i_kind)       :: ilat,ilon
  integer(i_kind),dimension(5):: idate5
  integer(i_kind)       :: instr,ichan, ierr
  ...
  real(r_double),allocatable,dimension(:) :: data1b8, channels(:)
...
! Set various variables depending on type of data to be read
  ...
  if(jsatid == 'tropics-01') then
     kidsat=709
  elseif (jsatid == 'tropics-03') then
     kidsat = 228
  elseif (jsatid == 'tropics-05') then
     kidsat = 263
  elseif (jsatid == 'tropics-06') then
     kidsat = 264
  elseif (jsatid == 'tropics-07') then
     kidsat = 284
  else 
...
! IFSCALC setup
  ...
  if (isfcalc==1) then
     instr=12                    ! This section isn't really updated.
...
! weighting between observations within a given thinning group.
  ...
  if (assim) then
     write(6,*)'READ_TMS:  data are set to be assimilated'
  else
     write(6,*)'READ_TMS:  data are not set to be assimilated'
  endif
...
! Reopen unit to satellite bufr file
  ...
  if(ierr /= 0) then
     write(6,*) 'READ_TMS error in openning the bufr file ', trim(infile), ', iostate = ', ierr
     return
  endif
  
 !Open BUFR table
 ! table_file = 'tms_bufr.table'      ! make table file name
 ! inquire(file=trim(table_file),exist=tmstab)
 ! if (tmstab) then
 !    write(6,*)'READ_TMS: Reading BUFR Table: ',trim(table_file)
 !    open(lnbufrtab,file=trim(table_file))
 !    call openbf(lnbufr,'IN',lnbufrtab)
 ! else
 !    call openbf(lnbufr,'IN',lnbufr)     
 !    write(6,*) 'READ_TMS ', trim(table_file), ' not found'
 !    return
 ! endif
 ...
 call datelen(10)
  ...
  hdr2b ='SAZA SANGST SOZA BEARAZ SOLAZI HMSL'
!  mnemonics in non-operational TROPICS bufr are a little different:
!  hdr1b ='SAID FOVN YEAR MONTH DAY HOUR MINU SECO CLATH CLONH'
!  hdr2b ='AGIND SOZA BEARAZ SOLAZI'  ! AGIND instead of SAZA

! test write tropics in text  
open (unit=10,file="tropics_out.txt",action="write")
...
!       Extract satellite id.  If not the one we want, read next record
        ...
        !if(jsatid == 'tropics-01') ksatid=709  ! temporay fix
        !write(6,*) "READ_TMS: ", jsatid, " ksatid: ", ksatid, " kidsat: ", kidsat
...
!       Extract observation location and other required information
        if(abs(bfr1bhdr(9)) <= 90._r_kind .and. abs(bfr1bhdr(10)) <= r360)then
           ...
           write(6,*)'READ_TMS:  WARNING: latitude and longitude are', bfr1bhdr(9), bfr1bhdr(10)           
           cycle read_loop
        end if
...
!       Extract date information.  If time outside window, skip this obs
        ...
        panglr = bfr2bhdr(2)*deg2rad      ! satellite scan angle        
        ...
        if(abs(lza)*rad2deg > 50.0_r_kind) then
          nbad_lza = nbad_lza + 1
          cycle read_loop
        end if

!        solzen_save(iob)=zero        ! set to 0.0 to bypass CRTM check
        solzen_save(iob)=bfr2bhdr(3)  ! 
        solazi_save(iob)=bfr2bhdr(5) 

        !       non-operational TROPICS bufr:
        call ufbrep(lnbufr,channels,1,nchanl,iret,'CHNM')        
        call ufbrep(lnbufr,data1b8,1,nchanl,iret,'TMBR')

        bt_save(1:nchanl,iob) = data1b8(1:nchanl)

        ! test write tropics observations in text file
        if (iob < 1000) then
            write(6, '(F6.2, ",", F6.2, ",", F6.2, ",", F6.2, ",", F6.2)', advance='no') &
                 dlon_earth, dlat_earth, (lza) * rad2deg, (panglr) * rad2deg, ifov
            DO ichan=1,nchanl
               write(6, '(",", F10.0, ",", F10.6)', advance='no') channels(ichan), bt_save(ichan, iob)
            END DO
            write(6,*) ''  
         endif
         
         write(10, '(F6.2, ",", F6.2, ",", F6.2, ",", F6.2, ",", F6.2)', advance='no') &
                 dlon_earth, dlat_earth, (lza) * rad2deg, (panglr) * rad2deg, ifov
         DO ichan=1,nchanl
               write(10, '(",", F10.0, ",", F10.6)', advance='no') channels(ichan), bt_save(ichan, iob)               
         END DO
         write(10,*) ''  
...
if (nbad_lza > 0) then 
   write(6,*)'READ_TMS WARNING: there were ', nbad_lza, ' observations with lza > 50 deg: data not used'
endif
...
num_obs = iob-1
  ...
  if (num_obs <= 0) then
     write(6,*) 'READ_TMS: No TROPICS Data were read in'
     return
  else
     write(6,*) 'READ_TMS: Number of TROPICS Data read in, ', num_obs     
  end if
