module w_setup
  ...
  implicit none
  real(r_single),parameter::     missing = -9.99e9_r_single !clt moved here
  private
  public:: setup
...
!                           muse(k),muse(l)
  ...
  loop_for_all_obs: &
  do i=1,nobs
     skint=missing  !clt tothink
     sfcr=missing !clt tothink
     dtime=data(itime,i)
...
!              replace positions 17-22 with earth relative wind component information
           ...
           if(sfcr.ne.missing) sfcr=sfcr/r100
           call nc_diag_metadata("surface_roughness", sngl(sfcr))
